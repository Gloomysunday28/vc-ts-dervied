...
class Bullet {
  ...
  annotationDecoration: TextEditorDecorationType =
  ...
  generateInterface(bullet: BulletDto) {
    ...
    if (length) {
      const genericName = `${strUtils.uppcase(
        async ? name.slice(0, -1) : name
      )}ReturnType${async ? ">" : ""}`;
      ...
      bullet.hoverMessage.appendCodeblock(
        format.formatCode(content.slice(2), {
          prefix: `interface ${genericName} ${strUtils.filterStr(
            bullet.type,
            "{",
            "",
            "\n"
